...
def get_excel_sheet():
    ...
    table_path = f'{current_directory}/Tables/on-ground+online_tables_[TestTable].xlsx'   # retrieves excel sheet of test data
    sheet_name = 'on-ground' # Sheet name (currently default) [either will be "on-ground" or "online"].

    excel_data = pd.read_excel(table_path, sheet_name, dtype=str)
